...
export function usePasswordStrength(
    ...
    const announcement =
      value.length === 0
        ...
        : [
            ...
            guessable ? "Este é um padrão facilmente adivinhado." : "",
            unmet.length === 0
              ? "Todos os requisitos foram atendidos."
              : `Ainda falta: ${unmet.map((r) => r.label.toLowerCase()).join(", ")}.`,
          ]
            .filter(Boolean)
            .join(" ");
  ...
  useEffect(() => {
    // 0ms pro caso "vazio" em vez de um setSettled("") direto no corpo do
    // efeito: setState síncrono dentro de um efeito é sinalizado pelo
    // react-hooks/set-state-in-effect (cascata de renders); despachar via
    // setTimeout(0) preserva o comportamento (limpa assim que o valor muda)
    // sem cair na regra.
...
const TONES = {
  none: {
    bar: "bg-zinc-300 dark:bg-white/20",
    text: "text-zinc-500 dark:text-zinc-400",
  },
  danger: { bar: "bg-rose-500", text: "text-rose-600 dark:text-rose-400" },
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-zinc-200 dark:bg-white/12"
          >
            <motion.span
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-zinc-200 text-white dark:border-white/[0.16] dark:text-zinc-900">
                <motion.span
              ...
              <span className="sr-only">{rule.met ? "atendido" : "não atendido"}</span>
            </li>
          ))}
        </ul>
      )}
